...
void Shutdown()
{
    ...
    RenameThread("cryptofocus-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: cryptofocus.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: cryptofocusd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 30187 or testnet: 31187)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 30188 or testnet: 31188)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizecryptofocusamount=<n> " + _("Keep N CryptoFocus anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. CryptoFocus is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. CryptoFocus is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("CryptoFocus version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "CryptoFocus server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of CryptoFocus") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart CryptoFocus to complete") << "\n";
    ...
    nAnonymizeCryptoFocusAmount = GetArg("-anonymizecryptofocusamount", 0);
    if(nAnonymizeCryptoFocusAmount > 999999) nAnonymizeCryptoFocusAmount = 999999;
    if(nAnonymizeCryptoFocusAmount < 2) nAnonymizeCryptoFocusAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeCryptoFocusAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize CryptoFocus Amount %d\n", nAnonymizeCryptoFocusAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1FCS+1000 == (.1FCS+100)*10
       10FCS+10000 == (1FCS+1000)*10
    */
