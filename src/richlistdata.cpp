// Copyright (c) 2015 The CryptoFocus developers
		      ...
		      if(!fInvalid)
		      {
			    ...
			    { 
			        ...
			        if(ExtractDestination(atxout.scriptPubKey, dest))
			        {
			    	        ...
			    	        std::string cryptofocusAddress = CBitcoinAddress(dest).ToString();
				        CRichListDataItem richListDataItem;
				        richListDataItem.sAddress = cryptofocusAddress;
				        if(richListData.mapRichList.find(cryptofocusAddress) == richListData.mapRichList.end())
				            richListData.mapRichList.insert(make_pair(cryptofocusAddress, richListDataItem));
					
				        richListData.mapRichList[cryptofocusAddress].nBalance = richListData.mapRichList[cryptofocusAddress].nBalance - atxout.nValue;
