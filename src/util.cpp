...
int nAnonymizeCryptoFocusAmount = 500;
...
#else
    const char* pszModule = "cryptofocus";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CryptoFocus
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CryptoFocus
    // Mac: ~/Library/Application Support/CryptoFocus
    // Unix: ~/.CryptoFocus
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CryptoFocus";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "cryptofocus.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cryptofocusd.pid"));
