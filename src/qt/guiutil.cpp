...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("cryptofocus"))
        return false;
...
{
    // Convert cryptofocus:// to cryptofocus:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("cryptofocus://"))
    {
        uri.replace(0, 12, "cryptofocus:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CryptoFocus.lnk";
}
...
{
    return GetAutostartDir() / "cryptofocus.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("CryptoFocus-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  cryptofocus-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("CryptoFocus-Qt"));
